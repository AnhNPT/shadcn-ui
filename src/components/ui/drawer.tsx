...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({ className, children, ...props }, ref) => {
    ...
    return (
        <DrawerPortal>
            <DrawerOverlay />
            <DrawerPrimitive.Content ref={ref} className={cn("fixed z-50 flex h-auto flex-col border bg-background", (!direction || direction === "bottom") && "inset-x-0 bottom-0 mt-24 rounded-t-[10px]", direction === "right" && "top-0 right-0 w-screen max-w-80 h-full", direction === "left" && "top-0 left-0 w-screen max-w-80 h-full", className)} {...props}>
