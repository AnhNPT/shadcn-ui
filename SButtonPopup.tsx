...
const SButtonPopup = (props: ISButtonPopup) => {
    const { buttonLayout, children, poisition, buttonLayoutRender, disabledPopup, onOutsideClick, onButtonClick, buttonProps, wrapperProps, popupProps, clearDefaultClass, disabled, renderInput, popupSyncLayout } = props;
        ...
        <>
            <div {...wrapperProps} className={`sbutton-wrapper ${wrapperProps?.className || ""}`} ref={buttonRef}>
                {renderInput ? (
                    ...
                    <div {...buttonProps} className={`${clearDefaultClass ? "" : `sbutton-select ${disabled ? "disabled" : ""}`}`} onClick={disabled ? null : buttonProps?.onClick ? buttonProps?.onClick : handleOnButtonClick}>
                        {buttonLayoutRender ? buttonLayoutRender({ setToggleButton, toggleButton }) : buttonLayout}
                    </div>
                )}
            </div>
            <div {...popupProps} className={`sbutton-option shadow-primary ${popupProps?.className || ""} ${toggleButton === true ? "flex" : "hide"}`} ref={popperRef} style={{ ...popupProps?.style, ...styles.popper }} {...attributes.popper}>
